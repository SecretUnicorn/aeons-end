...
import { default_inserts, default_texts } from 'static/data/editorInserts';
...
function SendAppointmentDialog(props) {
  const { onClose, selectedValue, open, recipients, openComments } = props;
  const name = 'sendMail';
  ...
  const add_file = (event) => {
    ...
    raw_files.forEach((file, idx) => {
      const reader = new FileReader();
      reader.addEventListener('load', (e) => {
        setFiles([...files, { content: e.target.result, baseFile: file }]);
      });
  ...
  const B2MB = (b) => Math.round(b * Math.pow(10, -6) * 100) / 100;
    ...
    >
      ...
      <DialogContent>
        ...
        <Dialog
          ...
          classes={{ paper: props.classes.preview }}
              ...
              >
                ...
                <Grid item container direction="row" justify="space-around" spacing={1}>
                  ...
                  {defaultDurations.map((duration) => (
                    <Grid item container key={duration} xs={true} justify="center" spacing={1}>
                      <Grid item xs={true}>
                        <Button
